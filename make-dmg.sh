#!/bin/bash
# Build "The Reading Room.app" and package it into a distributable, drag-to-install
# disk image. If Developer ID signing + notarization credentials are available the
# DMG is also notarized and stapled; otherwise those steps are skipped (the DMG is
# still produced, but Gatekeeper will warn on other Macs).
#
...
#
# Usage: ./make-dmg.sh   ->  build/TheReadingRoom.dmg
set -euo pipefail
cd "$(dirname "$0")"

APP_NAME="The Reading Room"   # bundle + mounted-volume name
PROC="TheReadingRoom"         # running process / executable name
APP="build/${APP_NAME}.app"
DMG="build/${PROC}.dmg"       # spaceless file name -> clean download URL
rm -rf build/dmg-stage        # tidy leftovers from older runs

# Don't package (or re-sign) a running instance — a live app bundle locks files and
# makes `hdiutil` fail with "Resource busy".
if pgrep -x "$PROC" >/dev/null 2>&1; then
  echo "==> Quitting running ${APP_NAME}…"
  osascript -e "tell application \"${APP_NAME}\" to quit" >/dev/null 2>&1 || true
  for _ in $(seq 1 25); do pgrep -x "$PROC" >/dev/null 2>&1 || break; sleep 0.2; done
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/the-reading-room-dmg.XXXXXX")"
trap 'rm -rf "$WORK"' EXIT
STAGE="$WORK/stage"
TMP_DMG="$WORK/${PROC}.dmg"
...
# Capture codesign output first, then parse — piping codesign straight into an
